// Student note: Controller for vendor profile (loads from backend and saves edits)
...
class VendorProfileController extends ChangeNotifier {
  ...
  // Student note: form controllers (prefilled from API)
  ...
  // Student note: helper to safely get string
  String _s(dynamic v) => (v ?? '').toString();

  // Student note: helper to safely get int->string
  String _i(dynamic v) {
  ...
  Future<void> refresh() async {
    ...
    try {
      ...
      // ✅ IMPORTANT: these keys must be returned by backend:
      // contactName, companyName, email, phone, address, tankerCount, deliveries, logoUrl, vendorId
      ...
      location = address.trim().isNotEmpty ? address.trim() : "No Address";

      // Student note: prefill edit fields (important after register)
  ...
  Future<void> save(BuildContext context) async {
    try {
      ...
      // ✅ Student note: payload keys must match backend PATCH /vendors/profile/me
      final payload = {
      ...
      // Student note: update local fields immediately (UI feels faster)
      ...
      // Student note: refresh to get deliveries, vendorId, logoUrl from backend again
      await refresh();
    } catch (e) {
      if (context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
  ...
  Future<void> pickAndUploadPhoto(BuildContext context) async {
    try {
      ...
      final url = await VendorProfileService.uploadVendorPhoto(token, File(picked.path));
      logoUrl = url;
