...
import '../services/create_slot_service.dart';
...
class CreateSlotController extends AutoDisposeNotifier<CreateSlotState> {
  ...
  String _friendlyOverlapMessage(Map<String, dynamic> body) {
    ...
    return (body['error'] ??
            "Slot overlaps an existing slot. Please choose another time.")
        .toString();
  }
  ...
  Map<String, dynamic> _slotToUiMap(
    ...
    final tankerLiters = _asInt(
      slotJson['tankerCapacityLiters'],
      fallback: 12000,
    );

    final start =
        _parseToLocalDateTime(slotJson['startTime']) ?? DateTime.now();
    final end =
        _parseToLocalDateTime(slotJson['endTime']) ??
        start.add(const Duration(hours: 2));

    final dateLabel = _service.buildDateLabel(start);
    final timeRange = _service.buildTimeRange(start, end);

    final uiStatus = bookedCount >= bookingSlots && bookingSlots > 0
        ? 'FULL'
        : 'OPEN';
  ...
  Future<String?> publishSlot({
    ...
    if (bookingSlots == null || bookingSlots <= 0)
      return 'Please enter valid booking slots';
    ...
    if (liters == null || liters <= 0)
      return 'Please enter valid tanker capacity (liters)';
    ...
    try {
      ...
      for (final r in state.routes) {
        ...
        final sameDay =
            rDate != null && _dateOnly(rDate) == _dateOnly(routeDate);
