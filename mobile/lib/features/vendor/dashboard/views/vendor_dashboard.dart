...
class _VendorDashboardScreenState extends State<VendorDashboardScreen>
  ...
  Future<void> _updateRequestStatus(int bookingId, String status) async {
    ...
    try {
      final res = await ApiService.patch('/vendors/requests/$bookingId', {
        'status': status,
      });
  ...
  Widget build(BuildContext context) {
    final showCompany = companyName.trim().isNotEmpty
        ? companyName.trim()
        : "Vendor Company";
    final showContact = contactName.trim().isNotEmpty
        ? contactName.trim()
        : "Vendor";
    ...
    final recentRequests = requests
        .where((rq) => _isWithinLast24Hours(rq['createdAt']))
        .toList();
                  ...
                  Column(
                    children: recentRequests.map((rq) {
                      final int bookingId = (rq['bookingId'] is num)
                          ...
                          : int.tryParse(
                                  (rq['bookingId'] ?? rq['id'] ?? "0")
                                      .toString(),
                                ) ??
                                0;

                      final residentName = (rq['residentName'] ?? "Resident")
                          .toString();
                      ...
                      final isUpdating = _updatingBookingIds.contains(
                        bookingId,
                      );
            ...
            _bottomItem(
              ...
              onTap: () {
                Navigator.push(
                  context,
                  MaterialPageRoute(
                    builder: (_) => const VendorDeliveryHistoryScreen(),
                  ),
                );
              },
            ),
