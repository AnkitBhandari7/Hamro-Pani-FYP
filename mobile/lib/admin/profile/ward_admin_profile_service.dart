...
class WardAdminProfileService {
  ...
  static Future<Map<String, dynamic>> fetchProfile(String token) async {
    ...
    throw Exception('Failed to load profile: ${res.statusCode} - ${err['error'] ?? res.body}');
  }
  ...
  static Future<Map<String, dynamic>> updateProfile(String token, Map<String, dynamic> data) async {
    ...
    final err = _decodeJson(res);
    throw Exception('Failed to update profile: ${res.statusCode} - ${err['error'] ?? res.body}');
  }
  ...
  static Future<String> uploadPhoto(String token, File file) async {
    ...
    if (res.statusCode == 200) {
      ...
      if (url.isEmpty) {
        throw Exception('Upload succeeded but profileImageUrl missing: ${res.body}');
      }
      return url;
    }

    final err = _decodeJson(res);
    throw Exception('Photo upload failed: ${res.statusCode} - ${err['error'] ?? res.body}');
  }
}