...
class VendorDetailsController extends ChangeNotifier {
  ...
  Future<void> _init(Map<String, dynamic> vendor, int? slotId) async {
    ...
    statusLabel = st.isEmpty ? statusLabel : (st == 'AVAILABLE' ? 'Available Now' : st);
    ...
    bookingSlotsUsed = (vendor['slotsUsed'] is num) ? (vendor['slotsUsed'] as num).toInt() : bookingSlotsUsed;
    bookingSlotsTotal = (vendor['slotsTotal'] is num) ? (vendor['slotsTotal'] as num).toInt() : bookingSlotsTotal;
    ...
    if (slotId != null) {
      try {
        ...
        if (token != null && token.isNotEmpty) {
          final data = await VendorDetailsService.getSlotDetails(token: token, slotId: slotId);
